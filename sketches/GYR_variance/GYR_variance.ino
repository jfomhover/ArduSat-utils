...
//  accel.setFullResBit(true);
  ...
  gyro.reset();
  gyro.init(0x68); // instead of I2C_ADD_GYR
}

int count = 0;
int16_t gyro_x_array[PULL_SAMPLESIZE];
int16_t gyro_y_array[PULL_SAMPLESIZE];
int16_t gyro_z_array[PULL_SAMPLESIZE];
...
//  Serial.println("pullValues()");
  gyro.readGyroRaw(&(gyro_x_array[count]), &(gyro_y_array[count]), &(gyro_z_array[count]));
  count++;
}
...
void buildDataPacket() {
  ...
  data.gyro_x = gyro_x_array[count-1];

  data.format1 = DATATYPE_USERDEFINED_INT2;
  data.gyro_x_min = gyro_x_array[0];
  for (int i=1; i<count; i++)
    if (gyro_x_array[i] < data.gyro_x_min)
      data.gyro_x_min = gyro_x_array[i];

  data.format2 = DATATYPE_USERDEFINED_INT2;
  data.gyro_x_max = gyro_x_array[0];
  for (int i=1; i<count; i++)
    if (gyro_x_array[i] > data.gyro_x_max)
      data.gyro_x_max = gyro_x_array[i];
  ...
  float var = 0;
  for (int i=0; i<count; i++)
    var += (gyro_x_array[i]-avg)*(gyro_x_array[i]-avg);
  var = var / count;
  data.gyro_x_var = var;
  ...
  data.gyro_y = gyro_y_array[count-1];

  data.format6 = DATATYPE_USERDEFINED_INT2;
  data.gyro_y_min = gyro_y_array[0];
  for (int i=1; i<count; i++)
    if (gyro_y_array[i] < data.gyro_y_min)
      data.gyro_y_min = gyro_y_array[i];

  data.format7 = DATATYPE_USERDEFINED_INT2;
  data.gyro_y_max = gyro_y_array[0];
  for (int i=1; i<count; i++)
    if (gyro_y_array[i] > data.gyro_y_max)
      data.gyro_y_max = gyro_y_array[i];
  ...
  var = 0;
  for (int i=0; i<count; i++)
    var += (gyro_y_array[i]-avg)*(gyro_y_array[i]-avg);
  var = var / count;
  data.gyro_y_var = var;
  ...
  data.gyro_z = gyro_z_array[count-1];

  data.format11 = DATATYPE_USERDEFINED_INT2;
  data.gyro_z_min = gyro_z_array[0];
  for (int i=1; i<count; i++)
    if (gyro_z_array[i] < data.gyro_z_min)
      data.gyro_z_min = gyro_z_array[i];

  data.format12 = DATATYPE_USERDEFINED_INT2;
  data.gyro_z_max = gyro_z_array[0];
  for (int i=1; i<count; i++)
    if (gyro_z_array[i] > data.gyro_z_max)
      data.gyro_z_max = gyro_z_array[i];
  ...
  var = 0;
  for (int i=0; i<count; i++)
    var += (gyro_z_array[i]-avg)*(gyro_z_array[i]-avg);
  var = var / count;
  data.gyro_z_var = var;
}
