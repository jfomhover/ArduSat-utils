...
int count = 0;
int16_t accel_x_array[PULL_SAMPLESIZE];
int16_t accel_y_array[PULL_SAMPLESIZE];
int16_t accel_z_array[PULL_SAMPLESIZE];
...
//  Serial.println("pullValues()");
  accel.readAccel(&(accel_x_array[count]), &(accel_y_array[count]), &(accel_z_array[count]));
  count++;
}
...
void buildDataPacket() {
  ...
  data.accel_x = accel_x_array[count-1];

  data.format1 = DATATYPE_USERDEFINED_INT2;
  data.accel_x_min = accel_x_array[0];
  for (int i=1; i<count; i++)
    if (accel_x_array[i] < data.accel_x_min)
      data.accel_x_min = accel_x_array[i];

  data.format2 = DATATYPE_USERDEFINED_INT2;
  data.accel_x_max = accel_x_array[0];
  for (int i=1; i<count; i++)
    if (accel_x_array[i] > data.accel_x_max)
      data.accel_x_max = accel_x_array[i];
  ...
  for (int i=0; i<count; i++)
    avg += accel_x_array[i];
  avg = avg / count;
  data.accel_x_avg = avg;
  
  data.format4 = DATATYPE_USERDEFINED_FLOAT;
  float var = 0;
  for (int i=0; i<count; i++)
    var += (accel_x_array[i]-avg)*(accel_x_array[i]-avg);
  var = var / count;
  data.accel_x_var = var;


  // accel_Y
  data.format5 = DATATYPE_USERDEFINED_INT2;
  data.accel_y = accel_y_array[count-1];

  data.format6 = DATATYPE_USERDEFINED_INT2;
  data.accel_y_min = accel_y_array[0];
  for (int i=1; i<count; i++)
    if (accel_y_array[i] < data.accel_y_min)
      data.accel_y_min = accel_y_array[i];

  data.format7 = DATATYPE_USERDEFINED_INT2;
  data.accel_y_max = accel_y_array[0];
  for (int i=1; i<count; i++)
    if (accel_y_array[i] > data.accel_y_max)
      data.accel_y_max = accel_y_array[i];
  ...
  for (int i=0; i<count; i++)
    avg += accel_y_array[i];
  avg = avg / count;
  data.accel_y_avg = avg;
  
  data.format9 = DATATYPE_USERDEFINED_FLOAT;
  var = 0;
  for (int i=0; i<count; i++)
    var += (accel_y_array[i]-avg)*(accel_y_array[i]-avg);
  var = var / count;
  data.accel_y_var = var;


  // accel_Z
  data.format10 = DATATYPE_USERDEFINED_INT2;
  data.accel_z = accel_z_array[count-1];

  data.format11 = DATATYPE_USERDEFINED_INT2;
  data.accel_z_min = accel_z_array[0];
  for (int i=1; i<count; i++)
    if (accel_z_array[i] < data.accel_z_min)
      data.accel_z_min = accel_z_array[i];

  data.format12 = DATATYPE_USERDEFINED_INT2;
  data.accel_z_max = accel_z_array[0];
  for (int i=1; i<count; i++)
    if (accel_z_array[i] > data.accel_z_max)
      data.accel_z_max = accel_z_array[i];
  ...
  for (int i=0; i<count; i++)
    avg += accel_z_array[i];
  avg = avg / count;
  data.accel_z_avg = avg;
  
  data.format14 = DATATYPE_USERDEFINED_FLOAT;
  var = 0;
  for (int i=0; i<count; i++)
    var += (accel_z_array[i]-avg)*(accel_z_array[i]-avg);
  var = var / count;
  data.accel_z_var = var;
}
